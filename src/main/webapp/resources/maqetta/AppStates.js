...
//FIXME: Make sure node and statesList are always sent to getStyle
            ...
            setStyle: function (node, state, styleArray, silent) {
                ...
                if (!silent) {
                    connect.publish("/davinci/states/state/style/changed", [
                        {node: node, state: state, style: styleArray}
                    ]);
                }
...
//FIXME: Need to generalize this for nested states
            ...
            add: function (node, state, params) {
                ...
                connect.publish("/davinci/states/state/added", [
            ...
            remove: function (node, state) {
                ...
                connect.publish("/davinci/states/state/removed", [
                    {node: node, state: state}
                ]);
                this._updateSrcState(node);
            },
            ...
            rename: function (stateContainerNode, params) {
                ...
                if (!stateContainerNode || !stateContainerNode._maqAppStates || !stateContainerNode._maqAppStates.states || !stateContainerNode._maqAppStates.states.length) {
                    return false;
                }
                ...
                connect.publish("/davinci/states/state/renamed", [
                    {node: node, oldName: oldName, newName: newName, stateContainerNode: node }
                ]);
                return true;
            },
